/**
 * Finance review payout math. Keep in sync with backend/src/finance-payout-utils.ts
 */
...
export function shipmentPayoutSummary(shipment: {
  settlement?: { grossAmount?: number; deductions?: number } | null;
  load?: {
  ...
  const actualCarrierPayout = resolveActualCarrierPayout(
    shipment.settlement?.grossAmount,
    shipment.load?.finalPrice,
  );
  return buildFinancePayoutSummary({
    actualCarrierPayout,
    carrierAdvancePercent: resolveCarrierAdvancePercent(shipment.load),
    settlementDeductions: shipment.settlement?.deductions,
    completedAt: shipment.completedAt,
    physicalPodSubmittedAt: shipment.physicalPodSubmittedAt,
    advancePaymentReleasedAt: shipment.financeReview?.advancePaymentReleasedAt,
    paymentStatus: shipment.financeReview?.paymentStatus ?? "not_released",
  });
}
