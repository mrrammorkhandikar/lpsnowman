...
export default function AdminDriversPage() {
  ...
  const { data: drivers = [], isLoading: driversLoading } = useQuery<AdminDriver[]>({
  ...
  const buildDriverVerificationPayload = (data: DriverFormData) => ({
    licenseVerified: !!data.licenseImageUrl && licenseVerified,
    aadhaarVerified: !!data.aadhaarImageUrl && (aadhaarVerified || aadhaarDocVerified),
  });

  const createDriverMutation = useMutation({
    mutationFn: async (data: DriverFormData) => {
      const verification = buildDriverVerificationPayload(data);
      const res = await apiRequest("POST", "/api/admin/drivers", {
        name: data.name,
    ...
    onSuccess: () => {
      toast({ title: "Driver added successfully", description: "They can sign in with their username or email." });
    ...
    onError: async (error: Error) => {
      const message = error.message.includes(":")
        ? error.message.split(": ").slice(1).join(": ") || "Failed to add driver"
        : "Failed to add driver";
      toast({ title: "Failed to add driver", description: message, variant: "destructive" });
    },
  });
  ...
  const updateDriverMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<DriverFormData> }) => {
      ...
      const res = await apiRequest("PATCH", `/api/admin/drivers/${id}`, payload);
      return res.json();
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/admin/drivers"] });
      if (editingDriver?.id) {
        queryClient.invalidateQueries({ queryKey: [`/api/admin/drivers/${editingDriver.id}`] });
        queryClient.invalidateQueries({ queryKey: [`/api/admin/drivers/${editingDriver.id}/documents`] });
      }
    ...
    onError: async (error: Error) => {
      const message = error.message.includes(":")
        ? error.message.split(": ").slice(1).join(": ") || "Failed to update driver"
  ...
  const handleOpenEditDialog = (driver: AdminDriver, e?: React.MouseEvent) => {
    e?.stopPropagation();
    form.reset({
      name: driver.name,
      username: driver.username || "",
      phone: driver.phone,
      email: driver.email || driver.loginEmail || "",
      password: "",
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold" data-testid="text-drivers-title">Drivers</h1>
          <p className="text-muted-foreground">Manage {drivers.length} driver accounts — drivers sign in with username or email</p>
        </div>
          ...
          {filteredDrivers.map((driver) => (
            ...
            >
              ...
              <CardContent className="space-y-3">
                ...
                {(driver.username || driver.email || driver.loginEmail) && (
                  <div className="text-sm text-muted-foreground truncate">
                    Login: {driver.username || driver.email || driver.loginEmail}
                  </div>
                )}
                {(driver.email || driver.loginEmail) && (
                  <div className="text-sm text-muted-foreground truncate">
                    {driver.email || driver.loginEmail}
                  </div>
                )}
                ...
                <div className="flex justify-end gap-2 pt-2 border-t" onClick={(e) => e.stopPropagation()}>
                  <Button 
                    variant="ghost" 
                    size="sm"
                    onClick={(e) => handleOpenEditDialog(driver, e)}
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Login username" autoComplete="off" {...field} data-testid="input-driver-username" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {editingDriver ? "New password (admin only)" : "Password"}
                    </FormLabel>
                    <FormControl>
                      <Input
                        type="password"
                        placeholder={editingDriver ? "Leave blank to keep current password" : "At least 8 characters"}
                    ...
                    {editingDriver && (
                      <p className="text-xs text-muted-foreground">
                        Only admins can change a driver&apos;s password. Drivers cannot change it themselves here.
                      </p>
                    )}
                  </FormItem>
                )}
